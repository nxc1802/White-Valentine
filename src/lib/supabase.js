...
export async function insertPassword(pw) {
  const { data, error } = await supabase
    .from(PW_TABLE).insert({ password: pw.password, hint: pw.hint }).select().single();
  if (error) throw error;
...
export async function updatePassword(id, fields) {
  const { error } = await supabase.from(PW_TABLE)
    .update({ password: fields.password, hint: fields.hint }).eq('id', id);
  if (error) throw error;
}
...
const mapMessage = row => ({ id: row.id, text: row.text, displayOrder: row.display_order });
...
export async function insertGiftMessage(msg) {
  const { data, error } = await supabase
    .from(MSG_TABLE).insert({ text: msg.text, display_order: msg.displayOrder ?? 0 }).select().single();
  if (error) throw error;
