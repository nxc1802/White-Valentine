...
export function getLetters()   { if (!lettersPromise)   lettersPromise   = fetchAllLetters().catch(() => []);   return lettersPromise; }
export function getWishlist()  { if (!wishlistPromise)  wishlistPromise  = fetchAllWishlist().catch(() => []);  return wishlistPromise; }
export function getPasswords() { if (!passwordsPromise) passwordsPromise = fetchPasswords().catch(() => []);   return passwordsPromise; }
export function getPhotos()    { if (!photosPromise)    photosPromise    = fetchPhotos().catch(() => []);       return photosPromise; }
export function getSongs()     { if (!songsPromise)     songsPromise     = fetchSongs().catch(() => []);        return songsPromise; }
export function getMessages()  { if (!messagesPromise)  messagesPromise  = fetchGiftMessages().catch(() => []); return messagesPromise; }
