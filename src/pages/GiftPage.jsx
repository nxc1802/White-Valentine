...
let photoPool  = [];
function nextPhoto() {
  if (photoQueue.length === 0) photoQueue = [...photoPool].sort(() => Math.random() - 0.5);
  return photoQueue.pop();
}

/* ---- Petal Canvas (elegant light petals) ---- */
...
export default function GiftPage() {
  ...
  useEffect(() => {
    function spawn() {
      ...
      const isPhoto = canPhoto && (!canMsg || Math.random() > 0.40);
    ...
    setTimeout(() => { for (let i = 0; i < 8; i++) setTimeout(spawn, i * 200); }, 1200);
  ...
  const handlePhotoClick = useCallback((src) => {
    ...
    if (frameRef.current) {
      gsap.fromTo(frameRef.current, { scale: 0.88 }, { scale: 1, duration: 0.5, ease: 'back.out(2)' });
    }
  }, []);
    ...
    <div className="gift-page">
      ...
      <div className="falling-layer">
        {fallingItems.map(item => (
          <FallingItem key={item.id} item={item} onEnd={() => removeItem(item.id)} onPhotoClick={handlePhotoClick} />
        ))}
      </div>
      ...
      <div className="gift-content">
        ...
        <div className={`center-frame ${centerFlash ? 'flash' : ''}`} ref={frameRef}>
          ...
          <p className="frame-hint">Click ảnh đang rơi để chọn ✨</p>
        </div>
