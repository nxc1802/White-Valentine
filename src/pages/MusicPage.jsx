...
export default function MusicPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const selectedCovers = coverPool.length > 0 ? pickUniqueCovers(coverPool, songList.length) : [];
  ...
  return (
    <div className="music-page">
      ...
      <div className="viz-bg">
        {Array.from({ length: 55 }).map((_, i) => (
          <div key={i} className="viz-bar" style={{
            '--dur':   `${0.35 + Math.random() * 0.75}s`,
            '--delay': `${Math.random() * 0.75}s`,
            '--min':   `${3 + Math.random() * 7}px`,
            '--max':   `${18 + Math.random() * 80}px`,
          }} />
        ))}
      </div>
      ...
      {/* Now Playing */}
      <div className="now-playing" ref={playerRef}>
          ...
          >
            ...
            <div className="song-play-ind">
              {currentIdx === i && isPlaying
                ...
                : <span style={{ color: 'rgba(184,146,42,0.3)', fontSize: '0.9rem' }}>▶</span>
              }
            </div>
          </div>
        ))}
