...
const CARD_COLORS = [
  { bg: '#fffaf0', text: '#8b6914' },
  { bg: '#fdf8f2', text: '#7a5a50' },
  { bg: '#fff5f8', text: '#c97080' },
  { bg: '#f8f5ff', text: '#7060b0' },
  { bg: '#f0fdf8', text: '#5a9e80' },
];
const ICON_OPTIONS = ['💌','✨','💕','🌟','📸','🎁','🤍','🌸','🌺','💗','❤️','🥰','💛','🌹','🦋'];
const ROTATIONS    = [-3.5, -2, 1.8, 3, -1.2, 3.8, -2.8, 1.5, -3.8, 2.8];
...
function LetterModal({ letter, onClose, onEdit }) {
  ...
  useEffect(() => {
    ...
    gsap.from(cardRef.current, { opacity: 0, scale: 0.91, y: 28, duration: 0.4, ease: 'back.out(1.5)' });
  }, []);
      ...
      <div className="add-letter-modal" ref={formRef} onClick={e => e.stopPropagation()}>
        ...
        <form className="add-letter-form" onSubmit={handleSubmit}>
          ...
          <div className="form-field">
            ...
            <input className="form-input" placeholder="Ví dụ: Bức thư #3, Kỷ niệm..."
    ...
    <div className="letter-overlay" ref={overlayRef} onClick={close}>
      <div className="add-letter-modal" ref={formRef} onClick={e => e.stopPropagation()}>
        ...
        <form className="add-letter-form" onSubmit={handleSubmit}>
          ...
          <div className="form-field">
            <label className="form-label">Nhãn (tag)</label>
            <input className="form-input" placeholder="Ví dụ: Bức thư #3, Valentine..."
              value={form.tag} onChange={e => setForm(f => ({ ...f, tag: e.target.value }))} maxLength={40} />
          </div>
          <div className="form-field">
            <label className="form-label">Tiêu đề *</label>
          ...
          <div className="form-actions">
            <button type="button" className="btn-cancel" onClick={close}>Hủy</button>
            <button type="submit" className="btn-save">Gửi thư 💌</button>
          </div>
        </form>
      </div>
    </div>
  ...
  return (
    <div className="letter-page">
      ...
      <div className="board-header">
        ...
        <p className="page-subtitle">Heartfelt words from Đạt and Linh</p>
      ...
      <div className="letters-board">
        {loading && <div className="letters-loading">Đang tải thư... 💌</div>}
        {filtered.map(letter => (
          <div
            key={letter.id}
