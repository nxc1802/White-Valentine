...
function PasswordTab({ showToast }) {
    ...
    async function handleSave(form) {
        try {
            if (modal === 'add') { await insertPassword(form); showToast('✅ Đã thêm password'); }
            else { await updatePassword(modal.id, form); showToast('✅ Đã cập nhật password'); }
            invalidatePasswords(); setModal(null); load();
        } catch { showToast('❌ Lỗi khi lưu', true); }
    }

    async function handleDelete(id) {
        if (!confirm('Xóa password này?')) return;
        try { await deletePassword(id); invalidatePasswords(); showToast('🗑️ Đã xóa'); load(); }
    ...
    return (
        <div className="admin-section">
            ...
            {loading ? <div className="admin-loading">Đang tải</div>
             : items.length === 0 ? <div className="admin-empty"><div className="empty-icon">🔑</div><p>Chưa có password nào</p></div>
             : (
                <div style={{ overflowX: 'auto' }}>
                    <table className="admin-table">
                        <thead><tr><th>#</th><th>Password</th><th>Gợi ý</th><th></th></tr></thead>
                        <tbody>
                            {items.map((item, i) => (
                                <tr key={item.id}>
            ...
            {modal && <PasswordModal initial={modal === 'add' ? null : modal} onSave={handleSave} onClose={() => setModal(null)} />}
        </div>
    );
}
...
function PasswordModal({ initial, onSave, onClose }) {
    ...
    function handleSubmit(e) { e.preventDefault(); if (!password.trim() || !hint.trim()) return; onSave({ password: password.trim(), hint: hint.trim() }); }
    return (
        <div className="admin-modal-overlay" onClick={onClose}>
            <form className="admin-modal" onClick={e => e.stopPropagation()} onSubmit={handleSubmit}>
                ...
                <div className="admin-modal-btns">
                    <button type="button" className="btn-cancel" onClick={onClose} style={{ padding: '0.5rem 1.2rem', borderRadius: 10, border: '1.5px solid rgba(184,146,42,0.2)', background: 'rgba(253,247,238,0.7)', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', cursor: 'pointer' }}>Hủy</button>
...
const PHOTO_SECTIONS = [
    { key: 'solo',      label: '🌸 Ảnh cá nhân', desc: 'Dùng cho: Thư viện (solo), Ảnh bìa nhạc, Ảnh rơi Gift', type: 'solo',   category: 'gallery',    filter: p => p.type === 'solo' && p.category === 'gallery' },
    { key: 'couple',    label: '💕 Ảnh đôi',     desc: 'Dùng cho: Thư viện (couple)',                           type: 'couple', category: 'gallery',    filter: p => p.type === 'couple' && p.category === 'gallery' },
    { key: 'photobooth',label: '📷 Photobooth',   desc: 'Dùng cho: Trang Photobooth',                            type: 'couple', category: 'photobooth', filter: p => p.category === 'photobooth' },
];
...
function PhotosTab({ showToast }) {
    ...
    const load = useCallback(async () => {
        try { setLoading(true); setAllItems(await fetchPhotos()); }
    ...
    async function handleUpload(files, caption) {
        ...
        try {
            ...
            invalidatePhotos(); showToast(`✅ Đã upload ${files.length} ảnh`); load();
        } catch (err) { showToast('❌ Lỗi: ' + err.message, true); }
        setUploading(false); setModal(false);
    }
    ...
    async function handleDelete(item) {
        ...
        try {
            ...
            invalidatePhotos(); showToast('🗑️ Đã xóa ảnh');
        } catch { showToast('❌ Lỗi khi xóa', true); }
    }
    ...
    return (
        <div className="admin-section">
            <div className="admin-section-header"><h2>📸 Photos ({allItems.length} tổng)</h2></div>
            <div className="admin-photo-subtabs">
                {PHOTO_SECTIONS.map(s => (
                    <button key={s.key} className={`admin-photo-subtab ${photoSection === s.key ? 'active' : ''}`} onClick={() => setPhotoSection(s.key)}>
                        {s.label}<span className="subtab-count">{counts[s.key]}</span>
                    </button>
                ))}
            </div>
            <div className="photo-section-info">
                <span className="photo-section-desc">{section.desc}</span>
                <button className="admin-add-btn" onClick={() => { fileRef.current = null; setModal(true); }}>+ Upload ảnh</button>
            </div>
            {loading ? <div className="admin-loading">Đang tải</div>
             : items.length === 0 ? <div className="admin-empty"><div className="empty-icon">🖼️</div><p>Chưa có ảnh</p></div>
             : (
                <div className="admin-photo-grid">
                    {items.map(item => (
                        <div key={item.id} className="admin-photo-card">
                            <img src={item.src} alt={item.caption} loading="lazy" />
                            <div className="photo-overlay"><span className="photo-caption">{item.caption}</span></div>
                            <button className="admin-photo-del" onClick={() => handleDelete(item)}>✕</button>
                        </div>
                    ))}
                </div>
            )}
            {modal && <PhotoUploadModal sectionLabel={section.label} onUpload={handleUpload} onClose={() => setModal(false)} uploading={uploading} />}
        </div>
    );
}

function PhotoUploadModal({ sectionLabel, onUpload, onClose, uploading }) {
    const [files, setFiles]     = useState([]);
    const [caption, setCaption] = useState('');
    function handleSubmit(e) { e.preventDefault(); if (files.length === 0) return; onUpload(files, caption); }
    return (
        <div className="admin-modal-overlay" onClick={onClose}>
            <form className="admin-modal" onClick={e => e.stopPropagation()} onSubmit={handleSubmit}>
                <h3>📸 Upload → {sectionLabel}</h3>
                <label>Chọn ảnh</label>
                <input type="file" accept="image/*" multiple onChange={e => setFiles(Array.from(e.target.files))} style={{ display: 'block', background: 'none', border: 'none', padding: '0.5rem 0' }} />
                ...
                <div className="admin-modal-btns">
                    <button type="button" onClick={onClose} style={{ padding: '0.5rem 1.2rem', borderRadius: 10, border: '1.5px solid rgba(184,146,42,0.2)', background: 'rgba(253,247,238,0.7)', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', cursor: 'pointer' }}>Hủy</button>
...
function MessagesTab({ showToast }) {
    ...
    const load = useCallback(async () => {
        try {
            setLoading(true);
            setItems(await fetchGiftMessages());
            setReady(true);
        } catch (err) {
            if (err?.code === 'PGRST205' || err?.message?.includes('gift_messages')) setReady(false);
            else showToast('❌ Không tải được messages', true);
        } finally { setLoading(false); }
    }, [showToast]);
    useEffect(() => { load(); }, [load]);
    ...
    async function handleSave(form) {
        try {
            if (modal === 'add') { await insertGiftMessage({ text: form.text, displayOrder: items.length + 1 }); showToast('✅ Đã thêm'); }
            else { await updateGiftMessage(modal.id, { text: form.text, displayOrder: modal.displayOrder }); showToast('✅ Đã cập nhật'); }
            invalidateMessages(); setModal(null); load();
        } catch { showToast('❌ Lỗi khi lưu', true); }
    }

    async function handleDelete(item) {
        if (!confirm(`Xóa "${item.text}"?`)) return;
        try { await deleteGiftMessage(item.id); invalidateMessages(); showToast('🗑️ Đã xóa'); load(); }
        catch { showToast('❌ Lỗi khi xóa', true); }
    }
    ...
    return (
        <div className="admin-section">
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '1rem' }}>Các dòng chữ rơi trên trang <strong>Quà Valentine</strong>. Chỉnh sửa thoải mái ✨</p>
            {loading ? <div className="admin-loading">Đang tải</div>
             : !tableReady ? (
                <div className="admin-empty" style={{ textAlign: 'left' }}>
                    <div className="empty-icon">⚠️</div>
                    <p>Bảng <code>gift_messages</code> chưa được tạo</p>
...
create policy "Public delete" on gift_messages for delete using (true);`}</pre>
                    <button className="admin-add-btn" style={{ marginTop: '1rem' }} onClick={load}>🔄 Thử lại</button>
                </div>
             ) : items.length === 0 ? <div className="admin-empty"><div className="empty-icon">💬</div><p>Chưa có tin nhắn nào.</p></div>
             : (
            ...
            {modal && <MessageModal initial={modal !== 'add' ? modal : null} onSave={handleSave} onClose={() => setModal(null)} />}
        </div>
    );
}
...
function MessageModal({ initial, onSave, onClose }) {
    ...
    function handleSubmit(e) { e.preventDefault(); if (!text.trim()) return; onSave({ text: text.trim() }); }
    return (
        <div className="admin-modal-overlay" onClick={onClose}>
            <form className="admin-modal" onClick={e => e.stopPropagation()} onSubmit={handleSubmit}>
                <h3>{initial ? '✏️ Sửa tin nhắn' : '💬 Thêm tin nhắn'}</h3>
                <label>Nội dung</label>
                <input value={text} onChange={e => setText(e.target.value)} placeholder="VD: Happy White Valentine 🤍" autoFocus />
                <div className="admin-modal-btns">
                    <button type="button" onClick={onClose} style={{ padding: '0.5rem 1.2rem', borderRadius: 10, border: '1.5px solid rgba(184,146,42,0.2)', background: 'rgba(253,247,238,0.7)', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', cursor: 'pointer' }}>Hủy</button>
                    <button type="submit" className="admin-add-btn" disabled={!text.trim()}>{initial ? 'Lưu' : 'Thêm'}</button>
                </div>
            </form>
        </div>
    );
...
function MusicTab({ showToast }) {
    ...
    const load = useCallback(async () => {
        try { setLoading(true); setItems(await fetchSongs()); }
    ...
    async function handleAdd(form, file) {
        ...
        try {
            let src = form.src;
            if (file) src = await uploadSongFile(file);
            await insertSong({ title: form.title, artist: form.artist, src, displayOrder: items.length });
            invalidateSongs(); showToast('✅ Đã thêm bài hát'); setModal(false); load();
        } catch (err) { showToast('❌ Lỗi: ' + err.message, true); }
        setUploading(false);
    }
    ...
    async function handleDelete(item) {
        ...
        try {
            ...
            invalidateSongs(); showToast('🗑️ Đã xóa bài hát'); load();
        } catch { showToast('❌ Lỗi khi xóa', true); }
    }
    ...
    return (
        <div className="admin-section">
            ...
            {loading ? <div className="admin-loading">Đang tải</div>
             : items.length === 0 ? <div className="admin-empty"><div className="empty-icon">🎵</div><p>Chưa có bài hát nào</p></div>
             : (
                <div>
                    {items.map((item, i) => (
            ...
            {modal && <SongModal onSave={handleAdd} onClose={() => setModal(false)} uploading={uploading} />}
        </div>
    );
}
    ...
    return (
        <div className="admin-modal-overlay" onClick={onClose}>
            <form className="admin-modal" onClick={e => e.stopPropagation()} onSubmit={handleSubmit}>
                ...
                <div className="admin-modal-btns">
                    <button type="button" onClick={onClose} style={{ padding: '0.5rem 1.2rem', borderRadius: 10, border: '1.5px solid rgba(184,146,42,0.2)', background: 'rgba(253,247,238,0.7)', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', cursor: 'pointer' }}>Hủy</button>
                    <button type="submit" className="admin-add-btn" disabled={uploading}>{uploading ? 'Đang upload...' : 'Thêm'}</button>
                </div>
            </form>
        </div>
    );
...
export default function AdminPage() {
    ...
    function handleLogout() { sessionStorage.removeItem('admin_authed'); setAuthed(false); }
            ...
            <div className="admin-page">
                ...
                <div className="admin-gate">
                    ...
                    <form onSubmit={handleLogin}>
                        <input type="password" value={pw} onChange={e => setPw(e.target.value)} placeholder="Master password..." autoComplete="off" autoFocus />
                        <br />
                        <button type="submit" className="admin-add-btn" style={{ width: '100%', marginTop: '0.5rem' }}>Đăng nhập</button>
                    </form>
        ...
        <div className="admin-page">
            ...
            <div className="admin-dashboard">
                ...
                <div className="admin-tabs">
                    {[['passwords', '🔐 Passwords'], ['photos', '📸 Photos'], ['music', '🎵 Music'], ['messages', '💬 Messages']].map(([key, label]) => (
